...
for(let i=0;i<arr.length;i++){
    arr[i].addEventListener("click",(event)=>{
        // playSound(event.target.innerHTML);  event has property target so we'll get <button>w</button> and then we want the w by innerHTML...
        playSound(event.target.textContent.trim()) //because I use p tag inside button so with innerhtml i get <p>w</p> will get but i only want w so textContent.trim() use
    });

    document.addEventListener("keypress",(event)=>{
        playSound(event.key);   //keypress event has property key then we get innerHtml
    });
}